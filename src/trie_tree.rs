...
SparseMerkleTree<H, V, S>
{
    ...
    pub fn update(&mut self, key: H256, value: V) -> Result<&H256> {
        ...
        let node = MergeValue::trie_from_h256(0, key, value.to_h256());
        ...
        if self.root.is_zero() && !node.is_zero() { // empty root, build branch for root and update it
            let node = MergeValue::trie_from_h256(0, key, node.hash::<H>());
            ...
            if !node.is_zero() {
                self.insert_node(key, node, 0, MergeValue::trie_from_h256(self.height, self.root, self.root), &BranchKey::new(0, H256::zero()), prev_height)?
            } else { // delete
                self.delete_node(key, node, 0, MergeValue::trie_from_h256(self.height, self.root, self.root), &BranchKey::new(0, H256::zero()), prev_height)?
            };
    ...
    fn insert_node(&mut self, key: H256, node: MergeValue, distance: u8, current: MergeValue, parent_key: &BranchKey, prev_height: u8) -> Result<Option<H256>> {
        ...
        let node = MergeValue::trie_from_h256(height, key, node.hash::<H>());
        let current_key = BranchKey::new(distance, current.key());


        if let Some(parent_branch) = self.store.get_branch(&current_key)? { // not a leaf
            ...
            if target.is_zero() { // insert inplace
                let (new_left, new_right) = if key.is_right(distance) {
                    (another, target)
                } else {
                    (target, another)
                };
                ...
                self.insert_node(key, node, distance + 1, target, &current_key, prev_height)
            }
        } else { // no branch before
            let (main, sub) = if current.key() > key {
                (current.clone(), node)
            } else {
                (node, current.clone())
            };
            let (left, right) = if main.key().is_right(distance) {
                (main, sub)
            } else {
                (sub, main)
            };
            ...
            self.leaves_cnt.insert(distance, current_cnt + 2);
            let new_merge = merge_trie::<H>(self.height - distance - 1, &parent_key.node_key, &left, &right);

            // insert new branch
            let new_key = BranchKey::new(distance, new_merge.key());
        ...
        if !branch.is_none() {
            ...
            if left.key() == key || right.key() == key {
                ...
                if let Some(parent_branch) = self.store.get_branch(&parent_key)? {
                    ...
                    let remained_key = BranchKey::new(distance, remained.key());
                    if let Some(other_branch) = self.store.get_branch(&remained_key)? {
                        self.store.remove_branch(&BranchKey::new(0, self.root))?;
                        self.root = remained.hash::<H>();
                        self.store.remove_branch(&remained_key)?;
                        self.store.insert_branch(BranchKey::new(0, self.root), other_branch)?;
                    } else {
                ...
                // walk down the tree
                return self.delete_node(key, node, distance + 1, target, &current_key, prev_height);
            }
        } else {
            return Ok(None);
        }
    ...
    pub fn refresh_tree_hash(&mut self) -> Result<&H256> {
        if !self.root.is_zero() { // won't update empty tree
            let root_value = MergeValue::trie_from_h256(self.height, self.root, self.root);
    ...
    fn refresh_hash(&mut self, current: MergeValue, parent_node: H256, distance: u8) -> Result<MergeValue> {
        if let Some(branch) = self.store.get_branch(&BranchKey::new(distance, current.key()))? {
            let (left, right) = (branch.left, branch.right);
            let left = self.refresh_hash(left.clone(), current.key(), distance + 1)?;
            let right = self.refresh_hash(right.clone(), current.key(), distance + 1)?;
            let new_height = self.height - distance;
            let new_merge = merge_trie::<H>(new_height, &parent_node, &left, &right);
            ...
            if current.hash::<H>().is_zero() {
                Ok(MergeValue::zero())
            } else {
                Ok(MergeValue::trie_from_h256(self.height - distance, current.key(), current.hash::<H>()))
            }
        }
    }
